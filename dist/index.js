...
function createField(props) {
	...
	const setValue = (value2) =>
		props.control.setValue((prev) => {
			const newValue =
				typeof value2 === "function" ? value2(prev?.[props.name]) : value2;
	...
	const handleSubmit = async (onValid, onInvalid) => {
		try {
			...
			if (control.isValid()) {
				...
				control.focusError();
				setResponse(await onInvalid?.(value(), control));
			}
		} catch (e) {
			setResponse(e);
		} finally {
...
function createControl(props) {
	...
	const fieldArray = createMemo(() => [...fields().values()]);
...
function getErrorMap(control) {
	...
	function getErrorEntries(control2, name) {
		...
		for (const [fieldName, field] of control2.fields()) {
			list.push(
				...getErrorEntries(
					field,
