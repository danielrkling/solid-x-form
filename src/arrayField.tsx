import { Index, JSX, Setter, createMemo, onCleanup, onMount } from 'solid-js'

import { Control, FieldApi, Validate, createControl, createField, createFieldComponent } from '.'
      ...
      <Index each={field.value()}>
        {(item, index) => {
          ...
          const control: Control<TItem> = createControl({
            value,
            setValue,
            isSubmitted: field.isSubmitted,
            isSubmitting: field.isSubmitting,
            validate: props.validate,
          }) // Control object for managing the array item.

          onMount(() => field.control.registerField(index, control)) // Register the field on mount.
          onCleanup(() => field.control.unregisterField(index)) // Unregister the field on cleanup.
 ...
 */
export function createArrayMethods<TItem>(setValue: Setter<TItem[]>): ArrayMethods<TItem> {
  function append(item: TItem) {
    setValue(p => [...p, item]) // Append item to the end of the array.
  }

  function prepend(item: TItem) {
    setValue(p => [item, ...p]) // Prepend item to the beginning of the array.
  }
