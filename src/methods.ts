...
export function unregisterField<T>(parentControl: Control<T>, name: keyof T): void {
...
export function getErrorMap<T>(
  ...
  function getErrorEntries(control: Control<any>, name: string) {
    ...
    control.fields().forEach((value, key) => {
      list.push(...getErrorEntries(value, name ? name + '.' + String(key) : String(key)))
    })
    return list
  }
}

export function getInputProps<TParent, TKey extends keyof TParent = keyof TParent>(
  field: FieldApi<TParent, TKey, string>,
